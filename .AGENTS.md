...
Las clases de Tailwind (ej: `bg-primary`, `text-foreground`) están basadas en variables CSS (design tokens).

⚠️ Importante:

Estas variables **NO se definen en `tailwind.config.ts`**, sino en `global.css` dentro de `:root` y `.dark`.
...
* ❌ No modificar `tailwind.config.ts` para cambiar colores
* ❌ No usar colores hardcodeados (`bg-red-500`)
* ✅ Usar siempre tokens (`bg-primary`)
* ✅ Modificar colores únicamente en `global.css`
