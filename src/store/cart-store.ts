  ...
  persist(
    (set, get) => ({
      ...
      addToCart: (product: Product) => {
        ...
        // Generate a unique identifier for the product considering its selected variants (sorted by id)
        // This allows distinguishing different variants of the same product (e.g., with different sizes)
        const variantsPart = variants.length > 0
          ? `-${variants
            .map(v => v.id)
            .sort((a, b) => (a ?? '').localeCompare(b ?? ''))
            .join('-')}`
          : ''
        const cartItemId = `${product.id}${variantsPart}`
        const cart = get().cart
